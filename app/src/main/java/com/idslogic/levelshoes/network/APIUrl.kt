...
class APIUrl {
    companion object {
        ...
        fun getCategorySearch(storeCode: String) = //BASE_URL.plus(getStoreCode()).plus("/category/_search")
            BASE_URL.plus(storeCode).plus("/category/_search")
        fun getCategoryBasedTrendingProductsFromKlevuSearch() =
            BuildConfig.KLEVU_BASE.plus(klevuCloud).plus(klevuNSearch)
    }
